...
class cluster():
  ...
  def share_load(self, decision, delta_energy, deltaT, rate):
    ...
    if decision == "discharge":
      sorted_batteries = sorted(self.batteries, key=lambda b: (b.esitmatedSOH, b.currentSOC, b.currentEnergy), reverse=True)
    else:
      sorted_batteries = sorted(self.batteries, key=lambda b: (b.esitmatedSOH, -1*b.currentSOC, -1*b.currentEnergy), reverse=True)
